...
5c. **Corroboration invariant**: An answer MUST NOT be reported as corroborated — `Found` or `CorroboratedAbsent` — unless at least `CORROBORATION_FLOOR` independent peers other than the answering one AGREED with it. A pool holding fewer than `CORROBORATION_FLOOR` such peers MUST NOT attempt corroboration at all. **This binds every coin-record read the crate exposes, including a SCALAR (non-`_opt`) form** — a method is not exempt from this invariant merely because it never constructs an `OptAnswer`; it MUST be built as a fail-closed wrapper over the graded read rather than a second, ungraded path to the same data
